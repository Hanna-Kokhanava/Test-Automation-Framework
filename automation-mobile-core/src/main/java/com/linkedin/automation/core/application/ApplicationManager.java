...
public class ApplicationManager {
    ...
    public static void uploadApp(HostMachine hostMachine) {
        if (!FileManager.getInstance(hostMachine).isFileExist(APPIUM_FOLDER, appFile, getFileName())) {
            ...
            Logger.info(getFileName() + " is uploaded in " + APPIUM_FOLDER + " folder on " + hostMachine.getHostname());
        } else {
            Logger.info(getFileName() + " is already exist in " + APPIUM_FOLDER + " folder on " + hostMachine.getHostname());
        }
    }
